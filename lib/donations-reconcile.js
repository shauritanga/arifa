 ...
 *
 * The Selcom browser redirect and webhook are both best-effort. This sweep
 * makes correctness independent of them: it periodically re-asks Selcom for
 * the authoritative status of every unresolved donation and settles it (via
 * verifyDonation, which is the only path allowed to mark PAID and which
 * verifies the amount first). It also expires genuinely-abandoned checkouts.
 *
