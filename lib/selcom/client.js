...
import { mapSelcomStatus } from "./types";

const CREATE_ORDER_PATH = "/v1/checkout/create-order";
const ORDER_STATUS_PATH = "/v1/checkout/order-status";

/**
 * Billing address for the create-order payload. Selcom marks the billing block
 * mandatory, but our forms only collect name/email/phone — there is no postal
 * address to send. Card details are entered on Selcom's hosted page, so these
 * fields just need to be present and valid; we use ARIFA's fixed Tanzanian
 * location. Per-payer name and phone are filled in from the buyer.
 */
...
/**
 * Split a full name into first/last for Selcom's billing block. Everything
 * after the first token becomes the last name; a single-token name reuses it
 * for both so neither mandatory field is empty.
 */
function splitName(full) {
  const parts = toSelcomAscii(full).split(/\s+/).filter(Boolean);
 ...
 */
function decodeGatewayUrl(value) {
 ...
 * printable ASCII.
 */
export function toSelcomAscii(value) {
  return (value || "")
    .normalize("NFKD")
 ...
 * keyed by a unique order_id (Selcom dedupes), order-status is a read.
 */
async function selcomFetch(url, init, attempts = 2) {
  let lastErr;
 ...
 *
 * `redirect_url`/`cancel_url`/`webhook` are wired to our own callback/webhook
 * routes so Selcom bounces the payer back after payment and POSTs a
 * server-to-server notification. The order_id is embedded in the browser URLs
 * so the callback can resolve the payment. Settlement is still reconciled
 * authoritatively via the order-status endpoint (see getOrderStatus).
 */
export async function createOrder(params) {
  assertSelcomConfigured();
  const currency = params.currency || selcomConfig.currency;

  const appUrl = selcomConfig.siteUrl.replace(/\/+$/, "");
  const callbackBase = `${appUrl}/api/selcom/callback?order=${encodeURIComponent(params.orderId)}`;
  const returnUrl = callbackBase;
  const cancelReturnUrl = `${callbackBase}&cancel=1`;
  const webhookUrl = `${appUrl}/api/selcom/webhook`;

  const { firstname, lastname } = splitName(params.buyerName);
  const buyerPhone = normalizePhone(params.buyerPhone);
  const paymentMethods = params.paymentMethods || "ALL";

  // Full create-order requires the billing block (mandatory). Nested fields
  // are sent as flat dotted keys ("billing.firstname"), matching Selcom's SDK,
  // and are signed like any other field via buildSelcomHeaders.
  const payload = {
    ...
    "billing.address_1": BILLING_DEFAULTS.address1,
    "billing.city": BILLING_DEFAULTS.city,
    "billing.state_or_region": BILLING_DEFAULTS.region,
    "billing.postcode_or_pobox": BILLING_DEFAULTS.postcode,
    "billing.country": BILLING_DEFAULTS.country,
    "billing.phone": buyerPhone,
    buyer_remarks: toSelcomAscii(params.remarks || "ARIFA donation"),
    merchant_remarks: "ARIFA",
    no_of_items: 1,
  };
  ...
  try {
    ...
    const gatewayUrl =
      ...
      (token
        ? `${selcomConfig.baseUrl}/v1/checkout/payment-link?token=${encodeURIComponent(token)}`
        : undefined);
...
export async function getOrderStatus(orderId) {
  ...
  try {
    const url = `${selcomConfig.baseUrl}${ORDER_STATUS_PATH}?order_id=${encodeURIComponent(orderId)}`;
